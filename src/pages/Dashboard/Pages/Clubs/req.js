...
const ClubLists = () => {
  ...
  const [pagination, setPagination] = useState({limit:5, all:'', totalPage:'', status:STATUS.REQUESTED, name:''});
  const [page, setPage] = useState(1)

  const Get = async () => {
    setLoad(true)
    const b = await GET(org_uri+`/status?page=${page}&limit=${pagination.limit}&status=${pagination.status}&name=${pagination.name}`)
    setData(b.data.data);
                        ...
                        data.map((item, index) => {
                          return(
                            <tr className="hover:bg-gray-100 dark:hover:bg-gray-700" key={index}>
                                ...
                                <td className="py-2 px-6 text-sm font-medium text-gray-500  dark:text-white text-overflow">{item.description}</td>
