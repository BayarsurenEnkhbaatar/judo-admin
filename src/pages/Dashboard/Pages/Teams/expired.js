...
import ExpiredModal from '../../../../components/Modals/Teams/expired';

const TeamsExpired = () => {
  const [load, setLoad] = useState(false);
  const [data, setData] = useState([])
  const [pagination, setPagination] = useState({limit:5, all:'', totalPage:'', username:''});
  ...
  const Get = async () => {
    setLoad(true)
    const b = await GET(athlete_uri+`-expired?page=${page}&limit=${pagination.limit}&username=${pagination.username}`)
    setData(b.data.data);
